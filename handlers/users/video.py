...
async def start_dow(message: types.Message):
    await message.answer(text=f"Hello, {message.from_user.full_name}, Send me the link to the video and I'll send it to you as audio.")
...
async def dowload(message: types.Message, state: FSMContext):
    ...
    await message.answer(text="All downloaded hold the audio")
    try:
        await bot.send_audio(message.chat.id, audio)
        await bot.send_message(message.chat.id, text='')
    except:
        await message.answer(text="The file is too big")
